...
const CustomCalendar = () => {
  ...
  // Get appointments for the entire month instead of just the selected date
  ...
  const { data: appointments, isLoading } = useGetAppointmentsQuery({
    ...
    page: 1,
    limit: 100, // Increase limit to get all appointments for the month
  ...
  // Create appointments by date mapping for the entire month
...
const handleDateClick = (day) => {
    if (day !== null) {
        ...
        // Scroll after a slight delay to ensure state is updated
        setTimeout(() => {
            window.scrollTo({ 
    ...
    <div className="p-6 w-full max-w-6xl mx-auto bg-white shadow-lg rounded-lg">
      ...
      <div className="grid grid-cols-7 gap-1 w-full">
        {calendarDays.map((day, index) => {
          ...
          const hasAppointments = day !== null && appointmentCount > 0;
                ...
                <>
                  ...
                  {hasAppointments && (
                    <div className="absolute bottom-1 flex justify-center">
                      <div className="bg-[#92D0FE] text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
                        {appointmentCount}
                      </div>
                    </div>
                  )}
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-sm">
                        <Link to={`/message/${appt._id}`} className={`px-2 py-1 rounded-full cursor-pointer hover:bg-white text-xs ${appt.status === 'confirmed' ? 'bg-green-100 text-green-800' :
                          appt.status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
                          'bg-red-100 text-red-800'}`}>
                          View Inbox
                        </Link>
                          <Link to={`/appointments/${appt.appointmentId}`} className={`px-2 py-1 rounded-full cursor-pointer hover:bg-white text-xs ${appt.status === 'confirmed' ? 'bg-green-100 text-green-800' :
                          appt.status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
                          'bg-red-100 text-red-800'}`}>
                          View Appointment
                        </Link>
