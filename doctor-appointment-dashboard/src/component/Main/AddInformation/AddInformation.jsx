import { Button, Form, Input, Radio, Select, DatePicker, Upload, message, Checkbox, TimePicker } from "antd";
...
import { useCreateTeamMemberMutation, useCreateMemberScheduleMutation } from "../../../redux/features/product/teamApi";
...
const AddInformation = () => {
 ...
 const [createMemberSchedule] = useCreateMemberScheduleMutation();
...
const handleCreateSchedule = async (userId) => {
    try {
      ...
      if (scheduleType === 'one-time') {
        scheduleData.date = values.scheduleDate?.format('YYYY-MM-DD');
        scheduleData.startTime = values.startTime?.format('hh:mm a');
        scheduleData.endTime = values.endTime?.format('hh:mm a');
      } else {
        scheduleData.dayOfWeek = values.dayOfWeek;
        scheduleData.startTime = values.startTime?.format('hh:mm a');
        scheduleData.endTime = values.endTime?.format('hh:mm a');
        scheduleData.repeatRule = values.repeatRule;
      }

      await createMemberSchedule({ userId, scheduleData }).unwrap();
  ...
  const onFinish = async (values) => {
    try {
     ...
     const createdUserId = response.data.attributes.id
console.log(createdUserId)

 if (showScheduleForm) {
          await handleCreateSchedule(createdUserId);
        }
 ...
 <div className="space-y-4 mt-8">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Schedule Information</h1>
                <p className="text-gray-600 mt-1">Set up availability schedule</p>
              </div>
              <Checkbox 
            ...
            {showScheduleForm && (
              <div className="bg-gray-50 p-4 rounded-lg mb-4">
                <Form.Item name="scheduleType" label="Schedule Type" initialValue="one-time">
                  <Radio.Group 
                ...
                {scheduleType === 'one-time' ? (
                  ...
                  <>
                    ...
                    <Form.Item
                      name="repeatRule"
                      label="Repeat"
                      rules={[{ required: true, message: "Please select repeat rule!" }]}
                    >
                      <Select placeholder="Select repeat rule">
