...
interface Appointment {
  ...
  timeSlot: string;
  status: string;
  appointmentId?: string; // Added for consistency
}
...
const MessagePage: React.FC = () => {
  ...
  }, [filteredMessages, handleMarkAsSeen, scrollToBottom]);
  ...
  const handlePayment = async () => {
    try {
      ...
      if (!currentConversation?.appointment?._id) {
        dispatch(setError("No appointment ID found"));
        return;
      }

      const paymentData = {
        appointmentId: currentConversation.appointment._id,
        amount: "9.99",
      };
  ...
  const sendMessage = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      const messagePayload = {
        ...
        appointmentId: typeof currentConversation.appointmentId === 'string' ? 
          currentConversation.appointmentId : 
          currentConversation.appointmentId?._id || null,
        text: messageInput,
        msgByUserId: currentUserId,
        type: "text" as const,
        conversationId: currentConversation._id
  ...
  }, [messageInput, currentUserId, receiver?._id, scrollToBottom, currentConversation, dispatch, messages]);
      ...
      try {
        ...
        const handleMessages = (data: Message[]) => {
          ...
          const relevantMessages = data.filter(msg => 
            !currentConversation || msg.conversationId === currentConversation._id
          );
